...
func (t *Manager) onJoinRequest(c *msgbus.JoinRequest) {
	nodes := clusterdump.ConfigData.Get().Nodes
	node := c.Node
	labels := []pubsub.Label{
	...
	if slices.Contains(nodes, node) {
		...
		t.bus.Pub(&msgbus.JoinError{Node: node, Reason: err.Error()}, labels...)
	}
}
...
func (t *Manager) onLeaveRequest(c *msgbus.LeaveRequest) {
	...
	if !slices.Contains(nodes, node) {
		...
		t.bus.Pub(&msgbus.LeaveError{Node: node, Reason: err.Error()}, labels...)
	}
}

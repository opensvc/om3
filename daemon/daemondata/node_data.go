...
func (d *data) dropPeer(peer string) {
	...
	for p := range instance.ConfigData.GetByNode(peer) {
		...
		d.pub.Pub(&msgbus.InstanceConfigDeleted{Node: peer, Path: p}, append(peerLabels, pubsub.Label{"namespace", p.Namespace}, pubsub.Label{"path", p.String()})...)
	}
	for p := range instance.StatusData.GetByNode(peer) {
		instance.StatusData.Unset(p, peer)
		d.pub.Pub(&msgbus.InstanceStatusDeleted{Node: peer, Path: p}, append(peerLabels, pubsub.Label{"namespace", p.Namespace}, pubsub.Label{"path", p.String()})...)
	}
	for p := range instance.MonitorData.GetByNode(peer) {
		instance.MonitorData.Unset(p, peer)
		d.pub.Pub(&msgbus.InstanceMonitorDeleted{Node: peer, Path: p}, append(peerLabels, pubsub.Label{"namespace", p.Namespace}, pubsub.Label{"path", p.String()})...)
	}
	if v := node.MonitorData.GetByNode(peer); v != nil {
		node.DropNode(peer)
		// TODO: find a way to clear parts of cluster.node.<peer>.Status
		d.pub.Pub(&msgbus.NodeMonitorDeleted{Node: peer}, peerLabels...)

		daemonsubsystem.DropNode(peer)
		d.pub.Pub(&msgbus.DaemonCollectorUpdated{Node: peer}, peerLabels...)
		d.pub.Pub(&msgbus.DaemonDataUpdated{Node: peer}, peerLabels...)
		d.pub.Pub(&msgbus.DaemonDnsUpdated{Node: peer}, peerLabels...)
		d.pub.Pub(&msgbus.DaemonHeartbeatUpdated{Node: peer}, peerLabels...)
		d.pub.Pub(&msgbus.DaemonListenerUpdated{Node: peer}, peerLabels...)
		d.pub.Pub(&msgbus.DaemonRunnerImonUpdated{Node: peer}, peerLabels...)
		d.pub.Pub(&msgbus.DaemonSchedulerUpdated{Node: peer}, peerLabels...)
	}
	...
	d.pub.Pub(&msgbus.ForgetPeer{Node: peer}, peerLabels...)
}

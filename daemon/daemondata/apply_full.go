...
func (d *data) applyNodeData(msg *hbtype.Msg) error {
	...
	d.pub.Pub(&msgbus.NodeDataUpdated{Node: remote, Value: msg.NodeData}, peerLabel, labelFromPeer)
...
func (d *data) pubMsgFromNodeConfigDiffForNode(peer string) {
	...
	onUpdate := func() {
		if !reflect.DeepEqual(prev, next) {
			...
			d.pub.Pub(&msgbus.NodeConfigUpdated{Node: peer, Value: *next.DeepCopy()},
...
func (d *data) pubMsgFromNodeStatsDiffForNode(peer string) {
	...
	onUpdate := func() {
		if !reflect.DeepEqual(prev, next) {
			...
			d.pub.Pub(&msgbus.NodeStatsUpdated{Node: peer, Value: *next.DeepCopy()},
				pubsub.Label{"node", peer},
				labelFromPeer,
			)
		}
...
func (d *data) pubMsgFromNodeStatusDiffForNode(peer string) {
	...
	onUpdate := func() {
		...
		if !reflect.DeepEqual(prev.Labels, next.Labels) {
			d.pub.Pub(&msgbus.NodeStatusLabelsUpdated{Node: peer, Value: next.Labels.DeepCopy()}, labels...)
			changed = true
		}
		if changed || !reflect.DeepEqual(prev, next) {
			node.StatusData.Set(peer, next.DeepCopy())
			d.pub.Pub(&msgbus.NodeStatusUpdated{Node: peer, Value: *next.DeepCopy()}, labels...)
		}
	}
	onCreate := func() {
		d.pub.Pub(&msgbus.NodeStatusLabelsUpdated{Node: peer, Value: next.Labels.DeepCopy()}, labels...)
...
func (d *data) pubMsgFromNodeCollectorDiffForNode(peer string, current *remoteInfo) {
	...
	if !hasPrev || current.collectorUpdated.After(prevTimes.collectorUpdated) {
		...
		d.pub.Pub(&msgbus.DaemonCollectorUpdated{Node: peer, Value: *dCollector.DeepCopy()},
			pubsub.Label{"node", peer},
			labelFromPeer,
		)
		return
...
func (d *data) pubMsgFromNodeDaemondataDiffForNode(peer string, current *remoteInfo) {
	...
	if !hasPrev || current.daemondataUpdated.After(prevTimes.daemondataUpdated) {
		...
		d.pub.Pub(&msgbus.DaemonDataUpdated{Node: peer, Value: *a.DeepCopy()},
			pubsub.Label{"node", peer},
			labelFromPeer,
		)
		return
...
func (d *data) pubMsgFromNodeDnsDiffForNode(peer string, current *remoteInfo) {
	...
	if !hasPrev || current.dnsUpdated.After(prevTimes.dnsUpdated) {
		...
		d.pub.Pub(&msgbus.DaemonDnsUpdated{Node: peer, Value: *a.DeepCopy()},
			pubsub.Label{"node", peer},
			labelFromPeer,
		)
		return
...
func (d *data) pubMsgFromNodeListenerDiffForNode(peer string, current *remoteInfo) {
	...
	if !hasPrev || current.listenerUpdated.After(prevTimes.listenerUpdated) {
		...
		d.pub.Pub(&msgbus.DaemonListenerUpdated{Node: peer, Value: *found.DeepCopy()},
			pubsub.Label{"node", peer},
			labelFromPeer,
		)
		return
...
func (d *data) pubMsgFromNodeRunnerImonDiffForNode(peer string, current *remoteInfo) {
	...
	if !hasPrev || current.runnerImon.After(prevTimes.runnerImon) {
		...
		d.pub.Pub(&msgbus.DaemonRunnerImonUpdated{Node: peer, Value: *a.DeepCopy()},
			pubsub.Label{"node", peer},
			labelFromPeer,
		)
		return
...
func (d *data) pubMsgFromNodeSchedulerDiffForNode(peer string, current *remoteInfo) {
	...
	if !hasPrev || current.scheduler.After(prevTimes.scheduler) {
		...
		d.pub.Pub(&msgbus.DaemonSchedulerUpdated{Node: peer, Value: *a.DeepCopy()},
			pubsub.Label{"node", peer},
			labelFromPeer,
		)
		return
...
func (d *data) pubMsgFromNodeMonitorDiffForNode(peer string, current *remoteInfo) {
	...
	if !hasPrev || current.nmonUpdated.After(prevTimes.nmonUpdated) {
		...
		d.pub.Pub(&msgbus.NodeMonitorUpdated{Node: peer, Value: *localMonitor.DeepCopy()},
			pubsub.Label{"node", peer},
			labelFromPeer,
		)
		return
...
func (d *data) pubMsgFromNodeInstanceDiffForNode(peer string, current *remoteInfo) {
	...
	for _, s := range updates {
		if _, ok := previous.instConfigUpdated[s]; !ok {
			...
			d.pub.Pub(&msgbus.ObjectCreated{Path: toPath[s], Node: peer},
		...
		d.pub.Pub(&msgbus.InstanceConfigUpdated{Path: toPath[s], Node: peer, Value: *d.clusterData.Cluster.Node[peer].Instance[s].Config.DeepCopy()},
			pubsub.Label{"namespace", toPath[s].Namespace},
			pubsub.Label{"path", s},
			pubsub.Label{"node", peer},
			labelFromPeer,
	...
	for _, s := range removes {
		...
		d.pub.Pub(&msgbus.InstanceConfigDeleted{Path: toPath[s], Node: peer},
			pubsub.Label{"namespace", toPath[s].Namespace},
			pubsub.Label{"path", s},
			pubsub.Label{"node", peer},
			labelFromPeer,
	...
	for _, s := range updates {
		...
		d.pub.Pub(&msgbus.InstanceStatusUpdated{Path: toPath[s], Node: peer, Value: *d.clusterData.Cluster.Node[peer].Instance[s].Status.DeepCopy()},
			pubsub.Label{"namespace", toPath[s].Namespace},
			pubsub.Label{"path", s},
			pubsub.Label{"node", peer},
			labelFromPeer,
	...
	for _, s := range removes {
		...
		d.pub.Pub(&msgbus.InstanceStatusDeleted{Path: toPath[s], Node: peer},
			pubsub.Label{"namespace", toPath[s].Namespace},
			pubsub.Label{"path", s},
			pubsub.Label{"node", peer},
			labelFromPeer,
	...
	for _, s := range updates {
		...
		d.pub.Pub(&msgbus.InstanceMonitorUpdated{Path: toPath[s], Node: peer, Value: *d.clusterData.Cluster.Node[peer].Instance[s].Monitor.DeepCopy()},
			pubsub.Label{"namespace", toPath[s].Namespace},
			pubsub.Label{"path", s},
			pubsub.Label{"node", peer},
			labelFromPeer,
	...
	for _, s := range removes {
		...
		d.pub.Pub(&msgbus.InstanceMonitorDeleted{Path: toPath[s], Node: peer},
			pubsub.Label{"namespace", toPath[s].Namespace},
			pubsub.Label{"path", s},
			pubsub.Label{"node", peer},
			labelFromPeer,

...
// instance status, over an empty instance status cache: whether an address
// answers for the object is decided across the instances, and the cache is
// where the peers are read.
func dnsManager(t *testing.T) *Manager {
	instance.InitData()
...
// update feeds an instance status the way the daemon does: into the cache the
// peers are read from, then to the handler.
func update(m *Manager, c *msgbus.InstanceStatusUpdated) {
	value := c.Value
	instance.StatusData.Set(c.Path, c.Node, &value)
...
func TestADeadResourceDoesNotUnpublishTheAddress(t *testing.T) {
	for _, avail := range []status.T{status.Warn, status.Down} {
		t.Run(avail.String(), func(t *testing.T) {
			m := dnsManager(t)
			update(m, &msgbus.InstanceStatusUpdated{
...
// scopedIP is an address the configuration scopes, so each instance names a
// different one.
func scopedIP(node, addr string, avail status.T) *msgbus.InstanceStatusUpdated {
	return &msgbus.InstanceStatusUpdated{
...
// A scoped address of a stopped object stays out of the zone, which is what
// the whole filter is for. The instances name different addresses, so none of
// them is the object's own and only a serving one may answer.
func TestAStoppedObjectWithScopedAddressesAnswersForNothing(t *testing.T) {
	m := dnsManager(t)
	update(m, scopedIP("node1", "128.0.0.11", status.Down))
	update(m, scopedIP("node2", "128.0.0.12", status.Down))

	require.Empty(t, aRecords(m, "svc1.root.svc.cluster1."))
	require.Equal(t, []string{"128.0.0.11"}, aRecords(m, "svc1.root.svc.node1.node.cluster1."),
		"the node affine name still answers for the instance on that node")
}

// An address every instance names is the object's own, so the object keeps
// answering with it while it is stopped: a client is told the connection is
// refused rather than that the name does not exist.
func TestAnAddressEveryInstanceSharesAnswersWhateverTheState(t *testing.T) {
	m := dnsManager(t)
	update(m, scopedIP("node1", "128.0.0.10", status.Down))
	update(m, scopedIP("node2", "128.0.0.10", status.Down))
	update(m, scopedIP("node3", "128.0.0.10", status.Down))

	require.Equal(t, []string{"128.0.0.10"}, aRecords(m, "svc1.root.svc.cluster1."),
		"staged once, by one instance, so the zone holds the address once")
	requireIndexInSync(t, m)
}

// Until the peers are known an address looks like the object's own. When one
// of them names a different address it is no longer, and the record staged
// for it goes.
func TestAPeerNamingAnotherAddressRetractsTheSharedRecord(t *testing.T) {
	m := dnsManager(t)
	update(m, scopedIP("node1", "128.0.0.11", status.Down))
	require.Equal(t, []string{"128.0.0.11"}, aRecords(m, "svc1.root.svc.cluster1."),
		"one instance, one address, nothing to disagree with it yet")

	update(m, scopedIP("node2", "128.0.0.12", status.Down))
	require.Empty(t, aRecords(m, "svc1.root.svc.cluster1."),
		"the addresses differ now, so neither is the object's own")
	requireIndexInSync(t, m)
}

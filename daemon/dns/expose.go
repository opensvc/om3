...
func ParseExpose(s string) (Expose, error) {
	...
	l := strings.Split(s, "/")
	if n := len(l); n != 2 {
		return expose, errors.Errorf("invalid expose syntax: expect one slash, got %d", n)
	}
	proto := l[1]
	...
	default:
		return expose, errors.Errorf("invalid expose syntax: expect network in tcp, tcp4, tcp6, udp, udp4, udp6. got %s", expose.Network)
	}

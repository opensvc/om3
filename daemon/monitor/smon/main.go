...
func (o *smon) worker(initialNodes []string) {
	...
	defer daemonps.UnSub(bus, daemonps.SubSvcAgg(bus, pubsub.OpUpdate, "smon agg.update", o.id, o.onEv))
	defer daemonps.UnSub(bus, daemonps.SubSetSmon(bus, pubsub.OpUpdate, "smon setSmon.update", o.id, o.onEv))
	defer daemonps.UnSub(bus, daemonps.SubSmon(bus, pubsub.OpUpdate, "smon smon.update", o.id, o.onEv))
	...
	for {
		...
		case i := <-o.cmdC:
			switch c := (*i).(type) {
			case daemonps.MonSvcAggUpdated:
				o.cmdSvcAggUpdated(c)
			case daemonps.SetSmon:
				o.cmdSetSmonClient(c.Monitor)
			case daemonps.SmonUpdated:
				o.cmdSmonUpdated(c)
			case cmdOrchestrate:
				o.needOrchestrate(c)
			}

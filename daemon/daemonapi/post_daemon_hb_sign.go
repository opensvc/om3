...
func localPostDaemonHeartbeatSign(ctx echo.Context, name api.InPathHeartbeatName) error {
	...
	if hbType != "disk" {
		log.Tracef("can't sign disk heartbeat %s: hb#%s.type is %s", name, hbType)
		return JSONProblemf(ctx, http.StatusBadRequest, "Invalid parameter", "can't sign disk heartbeat %s: hb#%s.type is %s", name, hbType)
	}

	devPath := config.GetString(key.New(section, "dev"))
	if devPath == "" {
		log.Warnf("can't sign disk heartbeat %s: hb#%s.dev is empty", name)
		return JSONProblemf(ctx, http.StatusBadRequest, "Invalid parameter", "can't sign disk heartbeat %s: hb#%s.dev is empty", name)
	}

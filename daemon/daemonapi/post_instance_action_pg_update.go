...
func (a *DaemonAPI) PostInstanceActionPGUpdate(ctx echo.Context, nodename, namespace string, kind naming.Kind, name string, params api.PostInstanceActionPGUpdateParams) error {
	if v, err := assertAdmin(ctx, namespace); !v {
		return err
	}
	nodename = a.parseNodename(nodename)
	if a.localhost == nodename {
		return a.postLocalInstanceActionPGUpdate(ctx, namespace, kind, name, params)
	}
	return a.proxy(ctx, nodename, func(c *client.T) (*http.Response, error) {
		return c.PostInstanceActionPGUpdate(ctx.Request().Context(), nodename, namespace, kind, name, &params)
	})
}
...
func (a *DaemonAPI) postLocalInstanceActionPGUpdate(ctx echo.Context, namespace string, kind naming.Kind, name string, params api.PostInstanceActionPGUpdateParams) error {
	...
	args := []string{p.String(), "instance", "pg", "update"}

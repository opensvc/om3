...
func (a *DaemonAPI) PatchObjectConfig(ctx echo.Context, namespace string, kind naming.Kind, name string, params api.PatchObjectConfigParams) error {
	...
	if _, ok := instanceConfigData[a.localhost]; ok {
		...
		if waiting {
			sub = a.subscribeConfigPropagation(fmt.Sprintf("api.patch_object_config %s %s", p, ctx.Get("uuid")), p)
		...
		// Asked to wait, the answer is held until the configuration has
		// reached every live node of the object, which is when an action
		// asked of any of them runs on it. The write is done either way: a
		// wait that expires says where the configuration has not landed yet,
		// and is not a failure of the write.
		if waiting && !mtime.IsZero() {
			if lagging := a.waitConfigPropagated(waitCtx, sub, p, mtime); len(lagging) > 0 {
	...
	for nodename := range instanceConfigData {
		var proxyOpts []funcopt.O
		if waiting {
			// The peer holds the answer for as long as the wait, which is
			// longer than a client allows an answer by default. The request
			// of the caller bounds it instead.
			proxyOpts = append(proxyOpts, client.WithTimeout(0))
		}
		c, err := a.newProxyClient(ctx, nodename, proxyOpts...)
		if err != nil {

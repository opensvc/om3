...
orchestration id, the object, or a pid you read from "om daemon ps".

Naming an exec rather than a pid is what makes signaling the wrong process
impossible. A pid a listing handed you may have exited and been recycled
since; the exec id is resolved to its pid by the daemon at the moment it
signals.
...
Example:
	...
	om daemon kill --pid 3924688 --signal=term
	om daemon kill 823c9944-fa2a-4c23-b921-3cc7398f0fc5
`,
	...
	flags.IntSliceVar(&options.Pids, "pid", nil, "signal the exec running under this pid")
...
func (t *CmdDaemonKill) Run() error {
	...
	if len(t.Pids) == 0 && t.ExecID == "" && t.SessionID == "" && t.OrchestrationID == "" &&
		len(t.Origins) == 0 && t.RID == "" && t.Selector == "" {
		return fmt.Errorf("name what to signal: an exec id, or one of --pid, --session-id, --orchestration-id, --rid, --origin, --service")
	}

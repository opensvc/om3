...
type (
	CmdObjectInstancePGReset struct {
		OptsGlobal
		commoncmd.OptsAsync
		commoncmd.OptsEncap
		commoncmd.OptsLock
...
func (t *CmdObjectInstancePGReset) Run(kind string) error {
	...
	return objectaction.New(
		...
		objectaction.WithRemoteFunc(func(ctx context.Context, p naming.Path, nodename string) (interface{}, error) {
			...
			params := api.PostInstanceActionPGResetParams{}
			...
			response, err := c.PostInstanceActionPGResetWithResponse(ctx, nodename, p.Namespace, p.Kind, p.Name, &params)
			if err != nil {
				return nil, err
			}
			switch {
		...
		objectaction.WithLocalFunc(func(ctx context.Context, p naming.Path) (interface{}, error) {
			type pgResetter interface {
				PGReset(context.Context) error
			}
			o, err := object.New(p)
			if err != nil {
				return nil, err

...
func TestKeywords(t *testing.T) {
	...
	t.Run("check default keywords value", func(t *testing.T) {
		...
		assert.Equal(t, []string{"/bin/true"}, app.StartCmd)
		assert.Equal(t, "", app.ScriptPath)
		assert.Equal(t, []string{}, app.StopCmd)
		assert.Equal(t, []string{}, app.CheckCmd)
	...
	t.Run("check custom keywords", func(t *testing.T) {
		...
		assert.Equal(t, []string{"/path1/demo.sh", "start", "106"}, app.StartCmd)
		assert.Equal(t, []string{"/path1/demo.sh", "stop", "106"}, app.StopCmd)
		assert.Equal(t, []string{"/path2/demo.sh", "status"}, app.CheckCmd)
